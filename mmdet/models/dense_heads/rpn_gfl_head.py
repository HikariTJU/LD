...
from mmdet.core import multi_apply, images_to_levels, distance2bbox
...
class RPNGFLHead(RPNTestMixin, AnchorHead):
    """RPN head.
    ...
    def __init__(self, in_channels, reg_max=16, **kwargs):
        self.reg_max = reg_max

        super(RPNGFLHead, self).__init__(1, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.rpn_reg = nn.Conv2d(self.feat_channels,
                                 self.num_anchors * (self.reg_max + 1) * 4, 1)
    ...
    def _get_bboxes(self,
        ...
        for idx, stride in zip(
                range(len(cls_scores)), self.anchor_generator.strides):
            ...
            rpn_bbox_pred = self.integral(rpn_bbox_pred) * stride[0]
            rpn_bbox_pred = rpn_bbox_pred.reshape(batch_size, -1, 4)
        ...
        batch_mlvl_proposals = distance2bbox(
            self.anchor_center(batch_mlvl_anchors),
